...
describe('only one argument string', () => {
  ...
  test('classnames with true condition', () => {
    expect(classnames(true && 'foo')).toStrictEqual({ className: 'foo' })
  })
  ...
  test('classnames with mixed value', () => {
    expect(classnames('a', 0, null, undefined, true, 1, 'b')).toStrictEqual({
      className: 'a 1 b',
    })
  })
})
...
describe('with multi argument string', () => {
  ...
  test('classnames with multi mixed condition', () => {
    expect(classnames(true && 'foo', false && 'baz', true && 'bar')).toStrictEqual({
      className: 'foo bar',
    })
  })

  test('classnames mixed', () => {
    expect(classnames(true && 'foo', 'baz', '', 'nice--hello')).toStrictEqual({
...
describe('only arrays', () => {
  test('classnames with multi array arguments', () => {
    expect(classnames(['foo bar baz'], [''], [''], ['bal'])).toStrictEqual({
      className: 'foo bar baz bal',
    })
  })

  test('classnames with multi mixed condition', () => {
    expect(classnames(true && ['foo'], false && ['baz'], true && ['bar'])).toStrictEqual({ className: 'foo bar' })
  })

  test('classnames mixed array with falsy value', () => {
    expect(classnames(['foo', null, '', false, 'bar', true && 'baz'])).toStrictEqual({
      className: 'foo bar baz',
    })
  })
})
...
describe('only one objects', () => {
  ...
  test('classnames with object with true value', () => {
    expect(classnames({ foo: true })).toStrictEqual({ className: 'foo' })
  })
...
describe('only multiple objects', () => {
  ...
  test('classnames with objects with mixed boolean value value', () => {
    expect(
      classnames({ foo: true, bar: false }, {}, { foo: false, bar: false }, {}, { foo: false, bar: true }, {})
  ...
  test('classnames with mixed falsy and truthy value', () => {
    expect(
      ...
      className: 'nonEmptyString whitespace function emptyObject nonEmptyObject emptyList nonEmptyList greaterZero',
    })
  })
})
